"""
该文件是Lora的具体实现,使用Roberta模型,定义了两个类,LoraRobertaSelfAttention and LoraWrapperRoberta.
"""
...
class LoraRobertaSelfAttention(RobertaSelfAttention):
    ...
    # 重写forward函数(参照父类的forward函数写法,只是将query和value的计算方式改为lora_query和lora_value(太硬编码了))
    def forward(
        self,
...
class LoraWrapperRoberta(nn.Module):
    ...
    def replace_multihead_attention_recursion(self, model):
        # model.named_children() 是 PyTorch 中用于遍历神经网络模型层的方法之一。
        for name, module in model.named_children():
            if isinstance(module, RobertaSelfAttention):
                ...
                # strict=False 表示允许忽略 LoRA 新层中的一些额外参数（即以 "lora_" 开头的参数），这些在原始层中不存在。
                ...
                # 将原始模型的该子模块替换为新的 LoraRobertaSelfAttention 层。setattr() 是 Python 的内置函数，它可以动态地设置对象的属性。name 是子模块的名字，这里相当于将 model.name 替换为 new_layer。
                setattr(model, name, new_layer)

            else:
                # 如果不是 RobertaSelfAttention 层，递归调用该函数，继续遍历子模块。
    ...
    # Optional是一个可选的参数，可以是None，也可以是一个值
